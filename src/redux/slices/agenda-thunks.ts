...
>('agendas/fetch', async (args = {}, thunkAPI) => {
...
export const fetchAgendaById = createAsyncThunk<
  ...
  string, // agendaId
  { rejectValue: string; state: RootState }
>('agendas/fetchById', async (agendaId, thunkAPI) => {
...
export const addAgenda = createAsyncThunk<
  ...
  CreateAgendaDto,
  { rejectValue: string; state: RootState }
>('agendas/add', async (payload, thunkAPI) => {
  const state = thunkAPI.getState() as RootState;
  const eventId = state.agendas.eventId;
...
export const updateAgenda = createAsyncThunk<
  ...
  { rejectValue: string; state: RootState }
>('agendas/update', async ({ agendaId, payload }, thunkAPI) => {
  const state = thunkAPI.getState() as RootState;
  const eventId = state.agendas.eventId;
...
export const deleteAgenda = createAsyncThunk<
  ...
  { rejectValue: string; state: RootState }
>('agendas/delete', async (agendaId, thunkAPI) => {
  const state = thunkAPI.getState() as RootState;
  const eventId = state.agendas.eventId;
